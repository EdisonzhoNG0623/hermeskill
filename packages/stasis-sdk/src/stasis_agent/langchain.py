...
- `on_tool_start` — record the call, update the loop-detection ring buffer,
  raise `StasisTerminated` if the apoptosis flag is set (M2 sets it)
...
`StasisTerminated` is raised — this is the L1 cooperative-termination
...
logger = logging.getLogger("stasis_agent.langchain")


def _checkpoint(state: WatcherState) -> None:
    """Raise StasisTerminated if the apoptosis flag is set on the given state.
    ...
    **First-cause wins:** if the flag is already set, subsequent Terminals
    ...
    for r in results:
        ...
        if isinstance(r, Terminal) and not state.terminate_requested:
            ...
            logger.warning(
                "stasis: agent %s entering apoptosis: %s (%s)",
                state.agent_id,
                r.symptom.value,
                r.reason,
            )
...
class StasisCallbackHandler(BaseCallbackHandler):
    ...
    # corresponding hook. `raise_error=True` lets `StasisTerminated` propagate
