"""LangGraph integration: attaches the Stasis callback to a compiled graph.

The plan originally talked about injecting `_stasis_gate` check nodes between
every node boundary. We don't actually need that — LangGraph fires
`on_chain_start` per node, and our `StasisCallbackHandler` raises
`StasisTerminated` from inside that callback when the apoptosis flag is set.
Same effect, but stays on LangChain's public callback API (not LangGraph's
internal node structure).

This module just provides `with_stasis(graph, handler)` — a thin wrapper that
...
def with_stasis(graph: Any, handler: StasisCallbackHandler) -> Runnable[Any, Any]:
    """Attach the Stasis callback handler to a graph (or any Runnable).
    ...
    if not hasattr(graph, "with_config"):
        raise TypeError(
            f"with_stasis() expects a compiled LangGraph or LangChain Runnable; "
