...
logger = logging.getLogger("stasis_agent.pricing")
...
def cost_for_usage(model: str, input_tokens: int, output_tokens: int) -> float:
    ...
    if price is None:
        if model not in _warned_models:
            logger.warning(
                "stasis_agent.pricing: unknown model %r; cost will be 0.0. "
    ...
    if age > STALE_THRESHOLD and key not in _warned_stale:
        logger.warning(
            "stasis_agent.pricing: price for %r is %d days old; consider refreshing.",
            model,
            age.days,
        )
