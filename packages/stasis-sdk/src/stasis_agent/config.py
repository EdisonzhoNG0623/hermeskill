"""SDK configuration: loads `~/.stasis/config.toml` + env-var overrides.

Resolution order (highest wins):
  1. explicit kwargs to `watch()` / `Client(...)`
  2. environment variables (STASIS_API_KEY, STASIS_BASE_URL)
  3. `~/.stasis/config.toml`
  4. built-in defaults
...
CONFIG_PATH = Path.home() / ".stasis" / "config.toml"
...
class SDKConfig(BaseModel):
    ...
    def load(cls) -> SDKConfig:
        ...
        # overwrite real env vars), so `stasis fleet` works without the user
        ...
        if env_url := os.environ.get("STASIS_BASE_URL"):
            data["base_url"] = env_url
        if env_key := os.environ.get("STASIS_API_KEY"):
