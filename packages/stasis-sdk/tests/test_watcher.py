...
The worker tests use a real StasisClient over httpx.MockTransport so we
...
from stasis_agent.types import EventType
...
def _client_capturing(calls: list[dict[str, Any]]) -> StasisClient:
    """A StasisClient that records every request to `calls`."""
