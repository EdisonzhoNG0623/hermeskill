...
Uses httpx.MockTransport for the registration HTTP call + a real
LangGraph StateGraph so the callback round-trip is exercised end-to-end
...
import httpx
import pytest
import stasis_agent._watch as watch_mod
from langgraph.graph import END, START, StateGraph
from stasis_agent import StasisTerminated, watch
from stasis_agent.client import AuthError, StasisClient
...
def _build_two_node_graph() -> Any:
    """A tiny LangGraph that goes START â†’ step_a â†’ step_b â†’ END."""
...
def _client_capturing(calls: list[dict[str, Any]], agent_id: str | None = None) -> StasisClient:
...
async def test_watch_registers_and_returns_invokable_graph() -> None:
    ...
    try:
        ...
        # 1. Registration POST happened with the right body
        ...
        # 2. The wrapped graph still runs the customer's logic
        assert result["counter"] == 2
        assert result["visited"] == ["a", "b"]

        # 3. A WatcherState was registered with the returned agent_id
        watchers = all_watchers()
        assert len(watchers) == 1
        assert str(watchers[0].agent_id) == agent_id
        assert watchers[0].name == "bot-v1"

        # 4. The BackgroundWorker is running
...
async def test_watch_records_registered_lifecycle_event() -> None:
    ...
    # After stop(), the worker has drained all pending events to the server.
    # Find the events POST and look for the registered lifecycle.
...
# --- apoptosis flag -> StasisTerminated ---------------------------------
...
async def test_terminated_flag_raises_at_chain_start() -> None:
    ...
    try:
        ...
        # Pretend M2 set the apoptosis flag
...
async def test_watch_propagates_registration_error_and_closes_owned_client() -> None:
    ...
    # No `client=` kwarg -> watch() owns the client and must close it.
    original_from_config = StasisClient.from_config
    closed: list[bool] = []

    def fake_from_config(*args: Any, **kwargs: Any) -> StasisClient:
        c = StasisClient(
            base_url="http://test",
            api_key="sk_bad",
            transport=httpx.MockTransport(handler),
    ...
    watch_mod.StasisClient.from_config = staticmethod(fake_from_config)  # type: ignore[method-assign]
    try:
        with pytest.raises(AuthError):
            await watch(_build_two_node_graph(), name="x", policy="coding-default")
    finally:
        watch_mod.StasisClient.from_config = original_from_config  # type: ignore[method-assign]
        await BackgroundWorker.stop()
...
def test_with_stasis_rejects_uncompiled_input() -> None:
    ...
    with pytest.raises(TypeError, match="compiled"):
        with_stasis("not a runnable", handler)  # type: ignore[arg-type]


@pytest.mark.asyncio
async def test_with_stasis_attaches_callback_to_runnable() -> None:
    """Without watch(), can manually wire a graph + handler for unit tests."""
    state = WatcherState(agent_id=uuid4(), name="t", policy=resolve_policy("coding-default"))
    handler = StasisCallbackHandler(state)
    graph = _build_two_node_graph()
    wrapped = with_stasis(graph, handler)

    # Running it should produce at least one lifecycle event in the state
    # (chain_start fires on the top-level node).
    await wrapped.ainvoke({"counter": 0, "visited": []})
    events = state.drain_events()
    # At minimum, chain_start was recorded
