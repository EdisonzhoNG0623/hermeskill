...
- Symptom events are queued with severity/reason/detail intact (these go
  to `stasis logs` + the M2.5 death cert)
...
from stasis_agent.langchain import StasisCallbackHandler, _apply_results
from stasis_agent.langgraph import with_stasis
from stasis_agent.policies import resolve_policy
from stasis_agent.types import EventType, Policy, PolicyThresholds, SymptomType
...
def test_tool_scope_violation_raises_at_tool_start() -> None:
    ...
    with pytest.raises(StasisTerminated, match="not in policy allowlist"):
        h.on_tool_start(
...
def test_symptom_event_payload_shape() -> None:
    """The symptom event powers death-cert + `stasis logs`. Verify the
...
def test_warning_is_recorded_with_warning_severity() -> None:
    ...
    from stasis_agent.checks import Warning
...
def test_already_dying_records_no_new_symptoms_for_same_cause() -> None:
    ...
    with pytest.raises(StasisTerminated, match="previously set"):
        h.on_tool_start(serialized={"name": "read_file"}, input_str="", run_id=uuid4())
...
def test_manual_flag_flip_raises_at_next_checkpoint() -> None:
    ...
    with pytest.raises(StasisTerminated, match="manual kill"):
...
def _looping_graph(state: WatcherState) -> Any:
    ...
    compiled = g.compile()
    return with_stasis(compiled, handler)
...
async def test_end_to_end_langgraph_dies_cooperatively_on_loop() -> None:
    ...
    with pytest.raises(StasisTerminated, match="repeated"):
