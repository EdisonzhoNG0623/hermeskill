...
async def test_watchdog_runs_in_separate_thread() -> None:
    ...
    wd_threads = [
        t for t in threading.enumerate() if t.name.startswith("stasis-watchdog-")
    ]
...
async def test_background_worker_stop_joins_watchdog_threads() -> None:
    ...
    from stasis_agent.watcher import register_watcher
    ...
    import httpx
    from stasis_agent.client import StasisClient
    from stasis_agent.watcher import ensure_worker_started
    ...
    wd_threads = [
        ...
        if t.name == f"stasis-watchdog-{state.agent_id}"
    ]
