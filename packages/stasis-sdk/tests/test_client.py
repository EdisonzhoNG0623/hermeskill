"""Unit tests for stasis_agent.client.StasisClient.
...
from stasis_agent.types import EventIn, EventType


def _client(handler: Any) -> StasisClient:
    return StasisClient(
        base_url="http://test",
        api_key="sk_test_xxx",
...
async def test_from_config_requires_api_key(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("STASIS_API_KEY", raising=False)
    monkeypatch.delenv("STASIS_BASE_URL", raising=False)
    # Point config loader at a non-existent file so the env-only fallback applies.
    monkeypatch.setattr(
        "stasis_agent.config.CONFIG_PATH",
        type("P", (), {"exists": staticmethod(lambda: False)})(),
    )
    # Disable the .env auto-loader; otherwise the test picks up the project's
    # real .env (which has STASIS_API_KEY set) and the AuthError never fires.
    monkeypatch.setattr(
        "stasis_agent.config._load_dotenv_into_environ", lambda *a, **kw: None
    )
    with pytest.raises(AuthError, match="STASIS_API_KEY"):
        StasisClient.from_config()
