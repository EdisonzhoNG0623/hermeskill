...
import pytest
from hermeskill.client import AuthError, TransportError
...
def _clean_registry() -> object:
    _reset_registry_for_tests()
    hermeskill_hermes._current_plugin = None
    yield
    _reset_registry_for_tests()
    hermeskill_hermes._current_plugin = None
...
def _offline_client() -> MagicMock:
    """Mock HermeskillClient whose register_agent fails as if the CP were down."""
    client = MagicMock()
...
async def test_setup_falls_back_to_local_when_control_plane_unreachable() -> None:
    ...
    with patch("hermeskill_hermes.plugin.ensure_worker_started") as worker:
        await plugin.setup()
...
async def test_setup_does_not_swallow_auth_errors() -> None:
    ...
    plugin = HermeskillPlugin(name="t", policy="coding-default", client=client)
    with (
        patch("hermeskill_hermes.plugin.ensure_worker_started"),
...
async def test_offline_async_register_wires_all_seven_hooks() -> None:
    ...
    with (
        patch("hermeskill_hermes.SDKConfig") as sdk_config,
        patch("hermeskill_hermes.HermeskillClient") as client_cls,
        patch("hermeskill_hermes.plugin.ensure_worker_started"),
    ):
        ...
        await hermeskill_hermes.async_register(ctx)
...
async def test_keyless_async_register_is_forced_offline() -> None:
    ...
    with (
        patch("hermeskill_hermes.SDKConfig") as sdk_config,
        patch("hermeskill_hermes.HermeskillClient") as client_cls,
        patch("hermeskill_hermes.plugin.ensure_worker_started") as worker,
    ):
        loaded = MagicMock()
        loaded.policy = None
