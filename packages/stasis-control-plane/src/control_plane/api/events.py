"""Events router: batched ingest from the SDK, paginated read for `stasis logs`.
...
- **Tailing** (`after_id` cursor): ascending order. Used by
  `stasis logs --follow` to poll for events newer than the last one seen.
"""
...
from stasis_agent.types import EventBatchIn, EventOut, EventPage, EventType
